...
join profiles p
on  u.id = p.user_id
where p.birthday > '2003-04-01' ;

select * from  users20;
-- ----------------------------------------------------------------------------------------------------------------------------------------

-- Найти кол-во,  отправленных сообщений каждым пользователем и  выведите ранжированный список пользователей, 
-- указав имя и фамилию пользователя, количество отправленных сообщений 
-- и место в рейтинге (первое место у пользователя с максимальным количеством сообщений) . (используя DENSE_RANK)

select
u.firstname,
u.lastname,
count(*) as amaunt_messages,
dense_rank() over (order by count(*)) as 'place in the ranking'
from users u
join messages m
on u.id = m.from_user_id
group by m.from_user_id;
-- ----------------------------------------------------------------------------------------------------------------------------------------
-- Вывести все сообщения, отсортировав сообщения по возрастанию даты отправления (created_at) 
-- и найти разницу дат отправления между соседними сообщениями, получившегося списка. ( LEAD или LAG)

select
id,
body,
created_at as created_sms,
lag(created_at) over(order by created_at) as next_sms,
(created_at - lag(created_at) over(order by created_at))/100 as 'разница. мин'
from
messages
